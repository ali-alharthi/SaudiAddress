...
class Districts extends Api
{
    ...
    public function all($cityId = 1, $lang = 'A')
    {
        ...
        if ($this->response == null) {
            ...
            if ($this->config->getCache()) {
                ...
                file_put_contents($cache, serialize($response['Districts']));
            }
            $this->response = $response['Districts'];
        }
    ...
    public function getId(int $districtId)
    {
        ...
        $key = array_search($districtId, array_column($this->response, 'Id'));
    ...
    public function getName($districtName)
    {
        ...
        $key = array_search($districtName, array_column($this->response, 'Name'));
